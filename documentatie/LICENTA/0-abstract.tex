...
\hspace{0.4cm} Aplicația WEB prezentată în această lucrare de licență se numește OfOps, aceasta având ca scop gestionarea spațiilor comune ale unei firme, astfel încât desfășurarea activităților de birou să fie optime.
...
The booking process can be done with a simple click on the desired spot. However, if there is already another reservation for that place, the user will be notified about it and will not be able to make it if another reservation is scheduled in the wanted interval. For meeting rooms, on the other hand, the user will receive a recommendation for an available meeting room for the desired time if the wanted meeting room is not available in the initial interval.

This paper will present all the stages that formed the basis of the application's development, from the initial design and its database structure to the APIs used for integrating its functionalities.
